...
export const getFileBlob = async (path: string): Promise<Blob | null> => {
    for (const candidate of resolveFileCandidates(path)) {
        const blob = await fetchFileBlob(candidate);
        if (blob) {
            return blob;
        }
    }
...
async function fetchFileBlob(path: string): Promise<Blob | null> {
    ...
    if (contentType.includes('application/json') || contentType.startsWith('text/')) {
        ...
        if (looksLikeJsonError(text)) {
            return null;
        }
        return new Blob([text], { type: contentType || 'text/plain' });
    }

    return response.blob();
}

function resolveFileCandidates(path: string) {
    ...
    if (clean) {
        ...
        if (clean.startsWith('data/assets/')) {
            candidates.add(clean.slice('data/'.length));
        }
    }
    return Array.from(candidates);
}

function looksLikeJsonError(text: string) {
    try {
        const parsed = JSON.parse(text);
        return typeof parsed === 'object' && parsed !== null && ('code' in parsed || 'msg' in parsed);
    } catch {
        return false;
    }
}
